...
const BannerProduct = () => {
  ...
  const deleteBanner = async (bannerId) => {
    ...
    // Use mock data
    try {
      const idx = bannerData.findIndex((b) => b.id === bannerId);
      if (idx !== -1) {
        bannerData.splice(idx, 1);
        setBanner([...bannerData]);
        toast.success("Banner deleted successfully (Mock)");
      } else {
        toast.error("Banner not found");
      }
    } catch (error) {
      toast.error("Failed to delete banner (Mock)");
      console.error("Error deleting banner (mock):", error);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const newBanner = {
        ...
        buttonText1: "",
        buttonText2: "",
        imgSrc: files[0] ? URL.createObjectURL(files[0]) : "/upload_area.png",
        description,
        imageUrl: files[0] ? URL.createObjectURL(files[0]) : "/upload_area.png",
      };
      bannerData.push(newBanner);
      setBanner([...bannerData]);
      toast.success("Added Successfully (Mock)");
      setName("");
      setDescription("");
      setFiles([undefined]);
    } catch (error) {
      toast.error("Something Wrong (Mock)");
      console.error("Error adding banner (mock):", error);
    } finally {
      setLoading(false);
    }
  };
                ...
                <tr key={index} className="border-t border-gray-500/20 ">
                  <td className="md:px-4 pl-2 md:pl-4 py-3 flex items-center space-x-3 truncate">
                    <div className="bg-gray-500/10 rounded p-2">
                      <Image
                        src={
                          (Array.isArray(banners.imageUrl) ? banners.imageUrl[0] : banners.imageUrl) || "/upload_area.png"
                        }
                        alt="product Image"
                        className="w-16"
                        width={1280}
