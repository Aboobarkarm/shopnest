...
import { DataCategories, productsDummyData } from '@/Data'; 
// import axios from "axios";
import { toast } from "react-toastify";

const AddProduct = () => {
  const [files, setFiles] = useState([undefined, undefined, undefined, undefined]);
  ...
  const handleSubmit = async (e) => {
    ...
    // Prepare product data from form fields
    const newProduct = {
      ...
      image: files
        .filter((file) => file)
        .map((file) => URL.createObjectURL(file)), // For mock, use local URLs
      date: Date.now(),
      __v: 0,
    };
    ...
    // Use mock data
    try {
      productsDummyData.push(newProduct);
      ...
      toast.error("Something Wrong");
      console.error("Error adding product (mock):", error);
    } finally {
      setLoading(false);
    }
  };
